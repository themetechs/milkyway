<?php
/**
 * The feedback factory class for themetechs SDK
 *
...
if ( ! class_exists( 'themetechs_SDK_Feedback_Factory' ) ) :
	/**
	 * Feedback model for themetechs SDK.
	 */
	class themetechs_SDK_Feedback_Factory {
		...
		/**
		 * themetechs_SDK_Feedback_Factory constructor.
		 *
		...
		public function __construct( $product_object, $feedback_types ) {
			if ( $product_object instanceof themetechs_SDK_Product && $feedback_types && is_array( $feedback_types ) ) {
				foreach ( $feedback_types as $type ) {
					$class                     = 'themetechs_SDK_Feedback_' . ucwords( $type );
